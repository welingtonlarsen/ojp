...
Timeout settings for acquiring slots provide backpressure when pools are exhausted. In the current single-timeout admission model, pooled lazy session allocation uses one admission budget, so fast and slow paths share the same timeout budget for admission. Slow query segregation still controls **which slot lane** an operation uses (fast vs. slow); it no longer introduces an additional timeout layer for backend pool borrow.

For pooled lazy session allocation, OJP uses a **single timeout budget at the admission semaphore**. In practice, `ojp.connection.pool.connectionTimeout` (and XA equivalent `ojp.xa.connection.pool.connectionTimeout`) is the user-facing wait budget for getting admitted. After admission, backend pool borrow is forced to fail fast. This avoids additive waits (admission wait + pool borrow wait) under saturation and keeps behavior consistent across XA, non-XA, and slow-query segregation paths.
